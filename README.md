...
scripts/cardPages.ts     build step: a page and a preview image per card, plus 404.html
...
The link preview images (`og/<card>.jpg`) are generated from the same artwork on every build, so new artwork shows up in previews with no extra step.
