...
- Sound and touch: real card sounds when a card opens, turns, lands or the theme changes, with a mute button in the toolbar. On phones a short vibration goes with each one, plus a light tick for every face that goes past while spinning a card (Vibration API on Android, the native switch haptic on iOS 18+).
...
src/
  ...
  assets/theme-star.svg  the star mask of the theme switch
  feedback.ts            sounds, vibration and the mute setting
  theme.ts               light and dark theme: state, persistence and the switch animation
  useKeyDown.ts          keyboard shortcuts (Esc, ← →)
  viewTransition.ts      animates between the grid and a card, when the browser supports it
  components/
    Toolbar/             language, sound and theme controls, fixed at the top right
...
| Colors of both themes, fonts, animation easing and the theme switch | `src/index.css` |
...
- Card sounds: [Kenney](https://kenney.nl/assets/casino-audio), CC0
